...
public enum ElectoralDistrict {
    ...
    public static Set<RegisteredVoter> votersIn(ElectoralDistrict district, Collection<RegisteredVoter> voters) {
        Set<RegisteredVoter> rv = voters.parallelStream().filter(voter -> voter.getElectorId().startsWith(district.getPrefix())).collect(Collectors.toSet());
    ...
    public static Set<Ballot> unspoiledBallots(Set<Ballot> votes) {
        return votes.stream().filter(vote -> !vote.isSpoiled()).collect(Collectors.toSet());
    }
